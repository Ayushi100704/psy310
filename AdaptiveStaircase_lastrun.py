...
expName = 'AdaptiveStaircase'  # from the Builder filename that created this script
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\ayush\\OneDrive\\Desktop\\Lab in Psych\\Classwork\\D3_exp3_AdaptiveStaircase\\AdaptiveStaircase_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    # --------Prepare to start Staircase "stait" --------
    # set up handler to look after next chosen value etc
    stait = data.StairHandler(startVal=10.0, extraInfo=expInfo,
        stepSizes=[1,1.25,1.5,2], stepType='lin',
        nReversals=0.0, nTrials=100.0, 
        nUp=1.0, nDown=3.0,
        minVal=1.0, maxVal=10.0,
        originPath=-1, name='stait')
    thisExp.addLoop(stait)  # add the loop to the experiment
    level = thisStait = 10.0  # initialise some vals
    
    for thisStait in stait:
        currentLoop = stait
        ...
        # store data for stait (StairHandler)
        stait.addResponse(key_resp.corr, level)
        stait.addOtherData('key_resp.rt', key_resp.rt)
